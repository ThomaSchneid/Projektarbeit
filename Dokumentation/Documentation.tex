...
\pagebreak
	...
	\section{Fazit}
		Die Grundidee ist plausibel und es ist auch möglich nur anhand der Intensitätswerte der Nachbarpixel Kanten und ihre Richtung zu berechnen. Die so gefunden Kanten sind allerdings noch sehr rau. Man muss die so gefundenen Kanten noch ausdünnen. Dünnt man die Kanten aus, nachdem der Algorithmus durchgelaufen ist, so müsste man noch zusätzliche Rechenzeit aufbringen, was den kompletten Prozess erneut verlangsamen würde. Man muss also einen Weg finden, bereits vor dem Berechnen einer Kante festzustellen, ob dieses Pixel berechnet werden muss oder nicht. Anhand einer geeigneten Überprüfung der Nachbarn sollte dies möglich sein. Mit der Geschwindigkeit des Canny Algorithmus konnten wir jedoch trotz allem nicht mithalten, die zusätzliche Verfolgung der Kanten bringt einen erheblichen zeitlichen Mehraufwand mit sich, welcher nicht durch die Einsparung der Pixel, welche man durch unsere Suchmethode weniger berechnen muss, auszugleichen ist.
