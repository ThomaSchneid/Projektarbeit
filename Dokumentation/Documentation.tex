...
\begin{document}
	...
	Wenn es um Bild- und Objekterkennung geht ist das menschliche Auge ungeschlagen. Innerhalb von wenigen Millisekunden erkennt es Kanten, klassifiziert und erkennt Objekte, ist imstande diese zu benennen und erkennt den Unterschied zwischen einer Zeichnung, einem Bild und der Realität. Es gibt viele Algorithmen zur Kantenerkennung, jedoch ist keiner von ihnen so Leistungsstark und Effizient wie das menschliche Auge. Nicht nur ist der Canny Algorithmus der wohl bekannteste, sondern auch der am meist benutzte.\nl
	Der Canny-Algorithmus benutzt mehrere Stufen um in kurzer Zeit zu einem faszinierenden Ergebnis zu gelangen. Nachdem ich meine Seminararbeit der Kantenverfolgung gewidmet habe, kam ich nicht umher, mit dem Canny-Algorithmus in Berührung zu kommen.
	Neben all der Faszination stellte sich mir schnell die Frage, wieso der Canny-Algorithmus so aufgebaut ist, wie er es ist. \nl
	Er durchläuft jedes Pixel eines Bildes mehrmals um so zum gewünschten Ergebnis zu kommen. 
	Diese Projektarbeit befasst sich mit Focus peaking, dem markieren scharf gestellter Objekte in einem Bild, und speziell dem Teilgebiet der Kantenverfolgung und ob sie die Performance aktueller Algorithmen verbessern kann. 
	Expliziter der Canny Algorithmus wird hier als Beispiel herangezogen, da er einer der besten Kantenerkennungsalgorithmen zurzeit ist.
	Ziel ist es, nicht das komplette Bild durchlaufen zu müssen, sondern - sobald eine Kante detektiert wurde - der Kante zu folgen und so schneller scharf gestellte Objekte zu erkennen. 
	Im Teilbereich der Hochgeschwindigkeitsfotografie könnte man damit schneller Objekte durch Autofokus scharf stellen und so die Bildqualität steigern.
