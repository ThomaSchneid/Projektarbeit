...
\newcommand{\selbst}{\quelle{Selbst erstellt mit diagrams.net von Google}}
\newcommand{\myname}{edge-tracker }
...
\begin{document}
	...
	Wenn es um Bild- und Objekterkennung geht, ist das menschliche Auge ungeschlagen. Innerhalb von wenigen Millisekunden erkennt es Kanten, klassifiziert Objekte, ist imstande diese zu benennen und erkennt den Unterschied zwischen einer Zeichnung, einem Bild und der Realität. Es gibt viele Algorithmen zur Kantenerkennung, jedoch ist keiner von ihnen so leistungsstark und effizient wie das menschliche Auge. Der wohl bekannteste und auch meist benutzte unter ihnen ist wohl der Canny Algorithmus.\nl
	Der Canny-Algorithmus benutzt mehrere Schritte, um in kurzer Zeit zu einem faszinierenden Ergebnis zu gelangen. Während ich meine Seminararbeit im Bereich der Kantenverfolgung geschrieben habe, kam ich nicht umhin auch erste Erfahrungen mit dem Canny-Algorithmus zu sammeln.
	Präzise und mit durchschnittlich unter einer halben Sekunde Rechenzeit für ein durchschnittliches, mit einer Digitalkamera aufgenommenes Bild auch extrem schnell findet dieser Algorithmus alle Kanten des Bildes, markiert diese und gibt das Kantenbildbild zurück.\nl
	...
	\begin{enumerate}
		...
		\item Vervollständigung der Kanten
	\end{enumerate}
	...
	Anschließend wird für jedes Pixel ein neuer, basierend auf den angrenzenden Nachbarn, Grauwert und der Gradient berechnet. Dies geschieht für jedes einzelne Pixel des Bildes.\nl
	Mit den so gefundenen Kantenpixeln wurden schon alle Kanten im Bild erkannt, allerdings sind diese noch sehr grob und unfein. Indem erneut alle Pixel des Bildes durchlaufen und die jeweiligen direkten Nachbarn in Gradientenrichtung geprüft und die entsprechenden Grauwerte gegebenenfalls angepasst werden, werden grobe, unfeine und breite Kanten ausgedünnt und feiner dargestellt.\nl
	Durch eine nochmalige Prüfung eines jeden Pixels werden die noch vorhandenen Kantenpixel klassifiziert und als starkes, schwaches oder kein Kantenpixel eingestuft, was das Kantenbild erneut verfeinert.\nl
	...
	Insgesamt durchlaufen wir das komplette Bild also mindestens 4 mal, um einzelne Kantenpixel zu finden, diese zu klassifizieren und im Nachhinein die eventuell falsch klassifizierte oder gefundene wieder zu korrigieren.\nl
	Genau hier greift meine Idee der Involvierung einer Kantenverfolgung an. Wenn wir bereits im ersten Durchlauf ein Kantenpixel finden, können wir die vollständige Kante durch verfolgen der Kantenrichtung und Erkennung der anderen Pixel auf dem Weg erhalten. Dies würde die Anzahl der Zugriffe auf ein Pixel reduzieren und damit die Geschwindigkeit des Algorithmus erhöhen.\nl
	...
	Im Folgenden erläutere ich die einzelnen Methoden des Canny Algorithmus mathematisch genauer, werde meine Ansätze sowie meinen Algorithmus \myname präsentieren und ihn direkt mit dem Canny Algorithmus vergleichen, um zu sehen, ob und welche Performancesteigerungen es gibt.
	...
	\section{Begriffsdefinitioen und Erklärungen}
		...
		\subsection{Was ist eine Kante}
			...
			Beim \myname Algorithmus setzen wir die Schwelle für einen Itensitätswertunterschied zur Erkennung eines Kantenpixels auf 20 und betrachten jeweils die 9 Nachbarn einer 8-Nachbarschaft sowie 2 Nachbarn einer 8-Nachbarschaft.
		...
		\subsection{Pixelnachbarn}
			...
			Wir benutzen im weiteren Verlauf stets die Achter-Nachbarschaft, da wir auch die diagonalen Nachbarn eines Pixels in die Kantenberechnung mit einbeziehen wollen, um spezifischere Ergebnisse zu erhalten.
		...
		\subsection{Bildrauschen}
			...
			\nlsubsubsection{Wie entsteht Rauschen in digitalen Bildern}
				...
				\nlparagraph{Quantenrauschen}					
					...
					Bei dunklen Bildern hingegen treffen weniger Photonen auf den Sensor, wodurch die zufällige Verteilung auf die Pixel extreme Abweichungen hervorrufen kann. Das Quantenrauschen ist bei dunkleren Bildern also stärker als bei hellen.\nl
					
				\nlparagraph{Beuteileigenschaften}
					Die auf dem Sensor angekommenen Photonen werden von einem Verstärker in Elektronen umgewandelt. Mit kleinen Kondensatoren wird die Ladung auf dem Chip gesammelt und in eine Spannung umgesetzt. Vor jeder neuen Bilderfassung müssen die Kondensatoren also "entleert" werden, wobei es zum sogenannten \textit{Reset-Noise} kommt. Das Entleeren geschieht mitnichten ideal oder gleichmäßig, wodurch es zu Restspannungen und damit einer zufälligen Schwankung am Ergebnis kommt. Diese Schwankung macht sich im Bild als Rauschen bemerkbar.\nl
					Weiterhin muss in vielen Schaltungen ein Ruhestrom fließen, welcher zusätzlich für ein Rauschen im Bild sorgt.\nl
	...
	\section{Canny-Algorithmus}
		...
		\subsection{Gauß Kern}
			Für die Faltung benutzen wir den oben bereits beschriebenen Gaußkern. Als Programmiersprache setzen wir Python ein und realisieren die Berechnung des Gauß Kerns folgendermaßen:
		...
		\subsection{Sobel Operator}
			...
			Für jedes Pixel werden die Komponenten der Matrix aufsummiert, um so den Grauwert zu erhalten.
		...
		\subsection{Non Maximum Suppression}
			Die Non Maximum Suppression Technik wird angewendet, um bereits gefundene Intensitätsmaxima (Kanten) erneut zu prüfen und diese auszudünnen. Hierfür durchläuft der Algorithmus jedes Pixel des Bildes und überprüft gefundene Kantenpixel, indem er sie mit ihren, basierend auf dem Gradienten des jeweiligen Pixels, direkten Nachbarpixelnvergleicht. Sollte eines der Nachbarpixel einen höheren Grauwert als das aktuelle Pixel aufweisen, wird der Grauwert des aktuellen Pixels auf 0 gesetzt.
			...
			Die Richtung der Kante wird durch den grünen Pfeil dargestellt und verläuft vertikal von oben nach unten. Der Algorithmus prüft nun, ob die Pixel ober- und unterhalb (im Bild blau umrahmt) des ausgewählten Pixels (im Bild rot umrahmt) einen höheren Intensitätswert aufweisen. In unserem Beispiel ist der Intensitätswert des unteren Pixels (i + 1, j) höher als der der anderen beiden (das untere Pixel ist weiß und hat damit einen Intensitätswert von 255). Der Intensitätswert des aktuellen Pixels wird nun auf 0 gesetzt. Hätte keines der beiden anderen Pixel einen höheren Intensitätswert, würde der aktuelle Wert beibehalten werden.\nl
...
\pagebreak
		...
		\subsection{Double threshold}
			...
			\begin{enumerate}
				\item \textbf{Starkes Pixel}
				\begin{itemize}
					\item Pixel, deren Intensitätswert hoch genug ist, dass wir uns sicher sein können, dass sie ein Teil der finalen Kante sind.\nl 
					Dieser Intensitätswert ist abhängig vom Gesamtbild. Wir multiplizieren den maximalen Intensitätswert des Bildes mit einem von uns gesetzten Faktor, hier 0.09,  um die Intensitätswertschwelle für ein starkes Pixel zu erhalten.
				\end{itemize}
				\item \textbf{Schwaches Pixel}
				\begin{itemize}
					...
					Diese Schwelle ist ebenfalls von Bild zu Bild verschieden, da hier die vom maximalen Intensitätswert des Bildes abhängige obere Schwelle mit einem von uns gesetzten Faktor, hier 0.05, multipliziert wird, um sie zu erhalten.
				\end{itemize}
	...
	\section{Kantenerkennung mit dem Canny Algorithmus}
		...
		\subsection{Noise Reduction}
			...
			\begin{figure}[H]
				...
				\quelle{https://web-done.de/wp-content/uploads/Canon-EOS-6D-ISO-102400-Testbild-12-346x300.jpg}
			\end{figure}
		
			Das verrauschte Bild wurde Kontrastreicher und Kanten dadurch deutlicher erkennbar.\nl
			Im Nachfolgenden werden wir unseren selbst gebauten Canny Algorithmus anhand des Hochschullogos testen. Hier entfällt der erste Schritt, da das Hochschullogo ein digital erzeugtes Bild ist und somit kein Rauschen beinhaltet.\nl
			...
			\begin{figure}[H]
				...
				\quelle{https://media.glassdoor.com/sqll/1115794/hochschule-karlsruhe-technik-und-wirtschaft-squarelogo-1450441508013.png}
			\end{figure}
			
		\subsection{Edge and Gradient Calculation}
			Nun wird berechnet, ob ein Pixel zu einer Kante gehört und gegebenenfalls die Richtung der Kante. Ob ein Pixel einer Kante zugehörig ist, wird anhand der Intensitätswertänderung bezüglich der umliegenden Pixel bestimmt. Hierfür werden die Intensitätsänderungen in horizontaler sowie vertikaler Richtung eines jeden Pixels bezüglich seiner direkten Nachbarn berechnet.\nl	
		...
		\subsection{Non Maximum Suppression}
			...
			Verglichen mit dem Bild aus dem vorherigen Schritt kann man erkennen, dass die Kanten deutlich dünner und detaillierter wurden. Man kann das Logo nun bereits gut nur anhand der Kanten erkennen. Die Intensitätswerte der nun gefundenen Kanten variieren jedoch noch stark. Viele Bildverarbeitungsprogramme oder auch Algorithmen basieren darauf, dass die gefundenen Kanten einen festen Intensitätswert aufweisen und verarbeiten das Bild dann anhand des angegebenen Intensitätswertes. Um auch unseren Kanten einen einheitlichen Intensitätswert zu geben, nutzen wir die folgenden zwei Schritte.\nl
		...
		\subsection{Double threshold}
			...
			\begin{enumerate}
				...
				\item schwaches Pixel, Intensitätswert wird auf den unteren Schwellenwert gesetzt.
			...
			Die starken Kanten sind heller und klarer erkennbar. Ebenso sind leichte, nach Anwendung der Non Maximum Suppression Methode, noch vorhandene Unfeinheiten oder einzelne Kantenpixel beseitigt worden.\nl
		...
		\subsection{Vergeich}
			...
			\begin{figure}[H]
				...
				\caption{\textit{Kantedektion mit dem \myname Algorithmus (links) und mit dem richtigen Canny (rechts)}}
			\end{figure}
	...
	\section{Effizienzgewinn durch Kantenverfolgung}
		\subsection{Idee}
			...
			\begin{figure}[H]
				...
				\caption{\textit{Testbild für den \myname Kantenverfolgungsalgorithmus}}
				\quelle{https://upload.wikimedia.org/wikipedia/commons/9/9f/Camera\_obscura.jpg}
			\end{figure}
		
			Dieses Bild enthält Kanten, welche nahe beieinander liegen, größere, grauwertemäßig nicht merklich unterschiedliche,  Flächen, weiter auseinanderliegende Kanten und Kanten unterschiedlicher breite, bis hin zu Linien. Es enthält zudem sowohl runde, als auch eckige oder kurvige Objekte. In diesem Bild sind die meisten Kantenformen enthalten, was es zu einem hervorragenden Testbild für unseren \myname Algorithmus macht.\nl
		...
		\subsection{Das Bild durchlaufen}
			...
			Wenn wir das Bild mit dieser Methode durchlaufen, gibt es allerdings einige Kanten, welche wir nicht finden können. Sollte eine Kante zwischen 2 roten Suchlinien in horizontaler Richtung verlaufen und unverzweigt sein oder befindet sich eine Kante in weniger als 3 Pixeln Abstand, unverzweigt und parallel zum jeweiligen Rand, so können wir diese nicht finden.\nl
			Wir wollen den \myname Algorithmus auf mit Digitalkameras aufgenommene, reale Bilder anwenden. Die Wahrscheinlichkeit, dass eine solche Kante im Bild vorkommt, sollte also verschwindend gering sein. Objekte in der Realität sind größer als 3 Pixel. Die Kanten eines Objekts sind verzweigt und schneiden somit unweigerlich eine der roten Linien. Wir können daher sicher sein, dass wir immer mindestens alle Objekte anschneiden und somit trotz dieser Einschränkungen alle Kanten des Bildes finden werden.\nl
			Die Höhe des Bildes in Pixeln sei y, die Breite x. Somit berechnet sich die Anzahl der durchlaufenen Pixel, ohne eine Kante zu verfolgen, zu:\nl
			
			\nlformelbox{Pixel\tief{ges} = $\frac{(y - 3)}{4}$ * (x - 6)}
			
			Im Verhältnis zu allen Pixeln des Bildes, prüfen wir somit folgenden Anteil des Bildes:\nl
			...
			Mit der Kantenverfolgung ist keine genaue Vorhersage möglich, da wir nicht im Vorfeld wissen, wie viele Kantenpixel sich im Bild befinden. Mit hoher Sicherheit kann man nur sagen, dass es weniger als alle Bildpixel sein werden, welche überprüft werden.\nl

		\subsection{\myname Algorithmus}
			Wir realisieren den \myname Algorithmus in Python. Folgende Funktion prüft das komplette Bild gemäß unserer oben beschriebenen Methode auf Kantenpixel und verfolgt gegebenenfalls eine gefundene Kante.\nl
			...
			Die Nachbarn mit der größten Differenz der gemittelten Grauwerte bestimmt die Richtung der Kante. Sollte keine Differenz über einem Schwellenwert liegen, welcher hier frei zu 20 gesetzt wurde, so handelt es sich nicht um ein Kantenpixel. Der Wert der Kantenrichtung wird nun im aktuellen Pixel des Whitescreens gespeichert.\nl
	...
	\subsection{Performance Steigerung des \myname Algorithmus}
		...
		\begin{figure}[H]
			\centering
			\includegraphics[width=0.4\linewidth] {../Bilder/mask_2_mediane_eff}
			\caption{\textit{Maske zur Untersuchung auf eine Kante in Richtung der 2. Mediane}}
			\selbst
		\end{figure}
		...
		\begin{figure}[H]
			\centering
			\includegraphics[width=0.4\linewidth] {../Bilder/mask_horizontal_eff}
			\caption{\textit{Maske zur Untersuchung auf eine Kante in horizontaler Richtung}}
			\selbst
		\end{figure}
		...
		\begin{figure}[H]
			\centering
			\includegraphics[width=0.3\linewidth] {../Bilder/mask_vertical_eff}
			\caption{\textit{Maske zur Untersuchung auf eine Kante in vertikaler Richtung}}
			\selbst
		\end{figure}
	
		Vergleichen wir nun das Ergebnis mit dem Vorherigen Ergebnis:\nl
		
		\begin{figure}[H]
			\centering
			\includegraphics[width=0.4\linewidth] {../Bilder/edge_finding_eff}
			\caption{\textit{Ergebnis der Kantensuche mit den neuen Masken}}
		\end{figure}
	...
	\section{Literaturverzeichnis}
		\begin{itemize}
			...
			\item \textit{https://upload.wikimedia.org/wikipedia/commons/9/9f/Camera\_obscura.jpg}
			\item \textit{https://de.linkfang.org/wiki/Nachbarschaft\_(Bildverarbeitung)}
			\item \textit{https://www.cosy.sbg.ac.at/~uhl/imgProcess.pdf}
			\item \textit{http://www9.in.tum.de/seminare/hs.SS06.EAMA/material/01\_ausarbeitung.pdf}
			\item \textit{https://de.wikipedia.org/wiki/Bildrauschen}
		\end{itemize}
\end{document}